...
function normalizeErrorResponse(status, bodyText) {
  if (!bodyText) {
    return 'Unable to create session. Please check your API key and workflow ID.';
  }
  ...
  if (status === 401 || status === 403) {
    return `${bodyText} (check that your API key has ChatKit beta access and is set on the server).`;
  }
...
async function handleSessionRequest(req, res, body) {
  ...
  try {
    ...
    if (!response.ok) {
      const errorMessage = normalizeErrorResponse(response.status, bodyText);
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'GET' && req.url === '/api/chatkit/config') {
    return sendJson(res, 200, { workflow: WORKFLOW_ID });
  }
