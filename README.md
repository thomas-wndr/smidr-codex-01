...
- Sample `.env.example` for your API key and workflow ID (auto-applied to the UI when present).
...
2. Run the server locally:
   ...
   node server.js
   ```
   Then open [http://localhost:3000](http://localhost:3000) and click **Start chat**. If `OPENAI_WORKFLOW_ID` is set in `.env`, the UI will use it automatically and hide the workflow input; otherwise you can paste a workflow ID manually.
...
- If you see `401`/`403` errors, confirm that the server has a valid `OPENAI_API_KEY` with ChatKit beta access and the correct `OPENAI_WORKFLOW_ID`. Hostinger may respond with a generic 403 if the Node app is not started or if the API call is blocked.
